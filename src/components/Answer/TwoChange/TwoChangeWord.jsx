...
export const TwoChangeWord = (props) => {
  ...
  const [result, setResult] = useState(textInput.text || ""); // Инициализируем result
  console.log(textInput, "QQQQQQQQQQQQQQQQQQ");

  const initialText = textInput.text || "";
  const Code = textInput.mkb10 || "";
  const [mkbcode, setMkbCode] = useState(Code);
  const [textValue, setTextValue] = useState(initialText); // Синхронизируем с result
  const [mkbValue, setMkbValue] = useState("");
  const current = new Date();
  const [info, setInfo] = useState("");
  ...
  const handleTextChange = (event) => {
    const newValue = event.target.value;
    setTextValue(newValue);
    setResult(newValue); // Синхронизируем result с textValue
    // debouncedChange убран, так как не определён. Если нужен, добавьте его.
  };
  ...
  const handleRestDiagnoz = async () => {
    ...
    try {
      ...
      if (response.data && response.data.data) {
        ...
        // Дописываем данные к текущему тексту (вместо замены)
        setTextValue((prev) => prev + (prev ? " " : "") + newData); // Добавляем пробел, если текст не пустой
        setResult((prev) => prev + (prev ? " " : "") + newData); // Синхронизируем result
      }
    } catch (error) {
      console.error("Ошибка при запросе:", error);
      alert("Ошибка при получении данных");
  ...
  const handleSave = () => {
    const newRecord = {
      ...
      name: JSON.stringify({
        ...
        time: cutTime(current),
        mkb10: info,
        text: result.trim(), // Используем result для сохранения
      }),
      order: 0,
      date: strDate(current),
      time: cutTime(current),
